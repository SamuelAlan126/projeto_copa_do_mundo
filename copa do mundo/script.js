...
function carregarJogos(jogosFiltrados = "") {
  ...
  // Se o dia é até 17 de junho, é a 1 rodada
  if (dia <= 17) {
    return "1º";
  }

  // Se o dia está entre 18 e 23, é a 2 rodada
  if (dia >= 18 && dia <= 23) {
    return "2º";
  }

  // Se o dia é 24 em diante, é a 3 rodada
...
let rodadaAtual = null;
  ...
  jogosFiltrados.forEach((jogo) => {
    ...
    const rodada = getRodada(jogo.data);
...
function configurarBusca() {
  ...
  inputBusca.addEventListener("input", () => {
    ...
    const textoFiltrado = inputBusca.value.toLowerCase();
    ...
    const jogosFiltrados = jogos.filter((jogo) => {
      ...
      return (
        jogo.time1.toLowerCase().includes(textoFiltrado) ||
        jogo.time2.toLowerCase().includes(textoFiltrado)
      );
    });
